export const insert_online_header = () => {
  ...
  if (online_div.length < 1) {
    ...
    const online_div =
      '<div class="availability--online"><h3>Available Online</h3><ul></ul></div>';
...
export const online_link_content = (link, target) => {
  const externalLinkIcon =
    "<lux-icon-base width='18' height='18' icon-name='Add Item'>\
                            ...
                            </lux-icon-new-tab></lux-icon-base>";
  return `Princeton users: <a href="${link}" target="${target}">View digital content${externalLinkIcon}</a>`;
};
...
export const insert_online_link = (
  ...
  if (existing_online_link.length > 0) return;
  let target = '_blank';
