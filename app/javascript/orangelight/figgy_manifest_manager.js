        ...
        resources.forEach((resource) => {
            if(this.isUnauthorizedSeniorThesis(resource)) {
                const content = (link, target) => {
                    return `<a href="${link}" target="${target}">Request a copy of ${resource.label}<i class="fa fa-external-link new-tab-icon-padding" aria-label="opens in new tab" role="img"></i></a><p>Princeton community has access to this thesis on campus or VPN.</p>`;
                };
