...
module Requests
  class RequestableDecorator
    delegate :system_id, :services, :charged?, :annex?, :on_reserve?,
             :aeon_request_url, :temp_loc_other_than_resource_sharing?, :call_number, :eligible_for_library_services?,
    ...
    def digitize?
      return false unless patron.core_patron_group? || (patron.affiliate_patron_group? && recap_edd?)
    ...
    def fill_in_digitize?
      return false unless patron.core_patron_group? || (patron.affiliate_patron_group? && recap?)
      !item_data? || digitize?
    end

    def pick_up?
      return false if !eligible_for_library_services? || (patron.affiliate_patron_group? && !off_site?)
