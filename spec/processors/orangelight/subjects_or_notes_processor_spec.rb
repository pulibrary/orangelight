...
RSpec.describe Orangelight::SubjectsOrNotesProcessor do
  ...
  context "when highlights are turned off" do
    before do
  ...
  context "when highlights are turned on" do
    before do
    ...
    context "when notes_display is present and subject_display is not" do
      before do
        allow(document).to receive(:highlight_field).with('lc_subject_display').and_return([])
        allow(document).to receive(:highlight_field).with('notes_display').and_return(values)
        allow(request_context).to receive(:action_name).and_return('index')
      end
      let(:values) { ['The lives of <em>Black</em> and Latino <em>teenagers</em> in a low-income :'] }
      context "notes_display field" do
