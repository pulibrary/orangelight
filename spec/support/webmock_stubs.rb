...
def stub_holding_locations
  stub_request(:get, "#{Requests.config['bibdata_base']}/locations/holding_locations.json")
    .to_return(status: 200,
               body: File.read(File.join(fixture_paths, 'bibdata', 'holding_locations.json')))
end

def stub_alma_holding_locations
  stub_request(:get, "#{Requests.config['bibdata_base']}/locations/holding_locations.json")
    .to_return(status: 200,
               body: File.read(File.join(fixture_paths, 'bibdata', 'alma', 'holding_locations.json')))
end

def stub_single_holding_location(location_code)
  file_path = File.join(fixture_paths, 'holding_locations', "#{location_code.tr('$', '_')}.json")
...
def stub_availability_by_holding_id(bib_id:, holding_id:, body: true)
  ...
  if body == false
    ...
    file_path = File.join(fixture_paths, 'availability', 'by_holding_id', "#{bib_id}_#{holding_id}.json")
    stub_request(:get, url)
      .to_return(status: 200, body: File.read(file_path))
  end
end
...
def stub_catalog_raw(bib_id:, type: nil, body: true)
  ...
  file_path = if type
                File.join(fixture_paths, 'raw', type, "#{bib_id}.json")
              else
                File.join(fixture_paths, 'raw', "#{bib_id}.json")
              end
  stub_request(:get, url)
    .to_return(status: 200, body: File.read(file_path))
end
