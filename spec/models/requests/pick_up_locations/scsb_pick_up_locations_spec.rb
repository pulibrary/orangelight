...
RSpec.describe Requests::PickUpLocations::ScsbPickUpLocations, :requests do
  it 'uses default_pick_ups from the form when the location has no pickup locations' do
    ...
    requestable = instance_double(Requests::Requestable, item: {}, location: {}, recap?: true)
  ...
  it 'does not include locations that are not valid pickup locations' do
    ...
    requestable = instance_double(Requests::Requestable, item: {}, location: { delivery_locations: [{ label: 'Mendel Music Library. Sound/Video', gfa_pickup: 'QK' }] }, recap?: true)
    locations = described_class.new(form:, requestable:)
