...
RSpec.describe Requests::ServiceEligibility::ClancyEdd, requests: true do
  ...
  describe '#eligible?' do
    ...
    describe 'by patron group' do
      let(:eligible_patron_groups) do
        ['P', 'REG', 'GRAD', 'SENR', 'UGRAD']
      end
      let(:ineligible_patron_groups) do
        ['Affiliate', 'Affiliate-P', 'CASUAL', 'GST']
      end
      it 'returns true if the patron is in an eligible patron group' do
        allow(requestable).to receive_messages(
            alma_managed?: true,
            held_at_marquand_library?: true,
            item_at_clancy?: true,
