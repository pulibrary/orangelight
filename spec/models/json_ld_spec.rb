  ...
  describe '#export_as_jsonld' do
    context 'with a standard MARC Record' do
      ...
      it 'creates a jsonld document' do
        ...
        expect(doc['title'].first['@language']).to eq('ko')
    ...
    context 'with an English-language journal' do
      ...
      it 'creates a jsonld document' do
        ...
        expect(doc['title'].first['@language']).to eq('en')
    ...
    context 'with no language code' do
      let(:id) { '9996490803506421' }

      it 'does not have a language' do
        expect(doc.keys).not_to include('language')
        expect(doc['language']).to be nil
        expect(doc['title'].first.keys).not_to include('@language')
      end
    end
