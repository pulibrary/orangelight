...
describe('AvailabilitySearchResults', function () {
  ...
  test('apply_scsb_record sets correct status for unavailable items', () => {
    const barcode = 'MR71868089';
    document.body.innerHTML = `
      <div data-scsb-availability="true" data-scsb-barcode="${barcode}"></div>
    `;
    ...
    searchResults.apply_scsb_record(barcode, item_data);

    const availability_element = document.querySelector(
      `*[data-scsb-barcode="${barcode}"]`
    );
    expect(availability_element.classList.contains('gray')).toBe(true);
    expect(availability_element.classList.contains('strong')).toBe(true);
    expect(availability_element.textContent).toBe('Request');
  });
