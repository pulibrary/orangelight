...
RSpec.describe BrowseLists::CallNumberCSV do
  ...
  describe "#run" do
    ...
    before do
      ...
      stub_request(
        :get, "http://127.0.0.1:8888/solr/orangelight-core-test/select?cursorMark=*&defType=edismax&facet=false&fl=call_number_browse_s,title_display,title_vern_display,author_display,author_s,id,pub_created_vern_display,pub_created_display,holdings_1display&indent=true&q=*:*&rows=500&sort=id%20asc&wt=json"
      )
        .to_return(status: 200, body: file_fixture("call_number_browse/index_entries_rows500_cursor_p1.json"), headers: {})

      stub_request(
        :get, "http://127.0.0.1:8888/solr/orangelight-core-test/select?cursorMark=AoEoMTAwMDA1NjY=&defType=edismax&facet=false&fl=call_number_browse_s,title_display,title_vern_display,author_display,author_s,id,pub_created_vern_display,pub_created_display,holdings_1display&indent=true&q=*:*&rows=500&sort=id%20asc&wt=json"
      )
    ...
    it "generates a csv" do
      _sql_command, facet_request, conn = BrowseLists.connection
      described_class.new(facet_request, conn, output_root: output_root, rows: 500).run
