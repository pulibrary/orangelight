...
RSpec.describe CatalogController, :type => :controller do
  ...
  it "authorized hostnames can access application" do
    allow_any_instance_of(Resolv).to receive(:getname).and_return('localhost')
    allow_any_instance_of(ApplicationController).to receive(:load_hostname_whitelist).and_return(['localhost'])    
    get :index
    expect(response).to have_http_status(200)
  end
  ...
  it "unuathorized users are redirected to library mainpage" do
    allow_any_instance_of(ActionDispatch::Request).to receive(:remote_ip).and_return('192.168.0.1')
    allow_any_instance_of(Resolv).to receive(:getname).and_return('google.com')
    get :index
